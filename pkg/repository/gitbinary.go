...
func rejectReceivePack(output io.Writer, input io.Reader, req *packp.UpdateRequests, reason error) error {
	var err error
	if req.Capabilities.Supports(capability.ReportStatus) || req.Capabilities.Supports(capability.ReportStatusV2) {
		w := output
		mux := req.Capabilities.Supports(capability.Sideband64k)
		if mux {
...
// pushGit pushes specs from dir to url with native git, streaming stderr to progress.
func (r *Repository) pushGit(ctx context.Context, dir, url string, specs []gitconfig.RefSpec, progress io.Writer) error {
	selects, err := r.pushSelectsRefs(specs)
	if err != nil {
		return err
	}
	if !selects {
		for _, spec := range specs {
			_, destination, _ := strings.Cut(spec.String(), ":")
			for _, pattern := range []string{spec.Src(), destination} {
				if err := runGitCmd(gitCmd(ctx, "", nil, "check-ref-format", "--allow-onelevel", "--refspec-pattern", pattern)); err != nil {
					return err
				}
			}
		}
		endpoint, err := transport.ParseURL(url)
		if err != nil {
			return err
		}
		session, err := client.New(mirrorClientOptions...).Handshake(ctx, &transport.Request{
			URL:     endpoint,
	...
	remote, config := mirrorRemoteConfig(url)
	args := []string{"-C", dir, "push", "--progress", "--", remote}
	for _, spec := range specs {
...
// pushSelectsRefs reports whether git gets a command to send or a source to reject; only well-formed wildcard updates matching no local ref do neither.
func (r *Repository) pushSelectsRefs(specs []gitconfig.RefSpec) (bool, error) {
	for _, spec := range specs {
		if spec.Validate() != nil || spec.IsDelete() || !spec.IsWildcard() {
			return true, nil
		}
	}
	...
	for name := range localRefs {
		for _, spec := range specs {
			if spec.Match(plumbing.ReferenceName(name)) {
				return true, nil
			}
		}
	}
