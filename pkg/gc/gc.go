...
func (c *Collector) Prune(ctx context.Context, opts PruneOptions) (*PruneResult, error) {
	...
	slog.InfoContext(ctx, "gc prune", "repositories", res.Repositories, "deleted_git_objects", res.DeletedGitObjects,
		"deleted_git_bytes", res.DeletedGitBytes, "reclaimed_bytes", res.ReclaimedBytes, "failed", len(res.Failed),
		"live", res.LiveObjects, "unlinked", len(res.Unlinked), "skipped_in_grace", res.SkippedInGrace, "dry_run", res.DryRun)
	if len(res.Failed) > 0 {
