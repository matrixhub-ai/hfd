...
func parseConfig() (*config, error) {
	...
	flag.BoolVar(&cfg.Internal, "internal", cfg.Internal, "Enable unauthenticated management endpoints under /internal/ (object listing under /internal/objects, GC via POST /internal/gc/prune then /internal/gc/sweep, which also reclaims shared git objects no repository reaches; run GC only while pushes are quiescent); expose only on trusted networks")
	flag.Parse()
